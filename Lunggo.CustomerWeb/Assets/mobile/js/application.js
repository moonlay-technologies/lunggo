    ...
    app.run(function ($rootScope, $log, $timeout) {
        ...
        $rootScope.PageConfig = {
            ...
            SetOverlay: function (overlay) {
                if (typeof (overlay) == 'undefined') {
                    $timeout(function() { // make it compatible for sync AND async function call
        ...
        $rootScope.FlightSearchForm = {
            ...
            Submit: function () {
               ...
               if ($rootScope.FlightSearchForm.DepartureDate == '') {
                ...
                var urlLink = origincity + '-' + destinationcity + '-' + $rootScope.FlightSearchForm.AirportOrigin.Code + '-' + $rootScope.FlightSearchForm.AirportDestination.Code + '/';
        ...
        $('.autocomplete-result ul.result').on('click', 'li', function (overlay) {
            ...
            if ($rootScope.FlightSearchForm.AutoComplete.Target == 'departure') {
                if ($rootScope.FlightSearchForm.AirportDestination.City == airport.City) $('.btnSubmit').addClass('disabled');
                else $('.btnSubmit').removeClass('disabled');
                $rootScope.FlightSearchForm.AirportOrigin = airport;
            } else {
                if ($rootScope.FlightSearchForm.AirportOrigin.City == airport.City) $('.btnSubmit').addClass('disabled');
                else $('.btnSubmit').removeClass('disabled'); 
                $rootScope.FlightSearchForm.AirportDestination = airport;
            }
        });
                                    ...
                                    if ($rootScope.DatePicker.HolidayNames[x].Year == year) {
                                        ...
                                        if (!foundMonth) {
                                            $rootScope.DatePicker.HolidayNames[x].Months.push({
                                                MonthName: month,
                                                Days: [{ Date: date, Name: holidayName}]
                                            });
                                        }
                                        break;
                                    }
        ...
        function getSelectedMonth(year, month) {
            ...
            } else if (!found) $('.holiday-list').remove();
        }
        ...
        //Some Initialisations used in Search
...
function getAuthAccess() {
    ...
    if (authKey) {
        ...
        if (token) return 1;
            ...
            else status = getAnonymousFirstAccess();
        }
    }
    return status;
}
