...
app.controller('siteHeaderController', ['$http', '$scope', '$log', function ($http, $scope, $log) {
    ...
    $scope.whatpage = function() {
        ...
        if (page == 'Flight') //|| $scope.page == 'Flight'
            return 1;
        else if (page == 'Hotel') //|| $scope.page == 'Hotel'lo
            return 2;
    }
    $scope.ProfileConfig = {
        getProfile: function () {
...
app.controller('accountController', [
    '$http', '$scope', '$log', function ($http, $scope, $log) {
        ...
        $scope.updateReservation = function(rsvNo, status) {
            ...
            if (authAccess == 2) {
                ...
                $http({
                    ...
                    if (refreshAuthAccess() && $scope.trial < 4) //refresh cookie
                    {
                        $scope.updateReservation(rsvNo, status);
                    }
                    else {
...
app.controller('registerController', [
    '$scope', '$http', '$log', function ($scope, $http, $log) {
        ...
        $scope.form.submit = function () {
            ...
            var authAccess = getAuthAccess();
            var regUrl = "";
            var checkUrl = "" + $scope.returnUrl;
            if (checkUrl.includes("B2B")) {
                regUrl = B2BRegisterConfig.Url;
            } else {
                regUrl = RegisterConfig.Url;
            }
            if (authAccess == 1 || authAccess == 2) {
                $http({
                    url: regUrl,
