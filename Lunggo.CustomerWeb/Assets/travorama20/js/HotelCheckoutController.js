...
app.controller('hotelcheckoutController', [
    '$http', '$scope', '$interval', '$location', '$log', function ($http, $scope, $interval, $location, $log) {
        ...
        $scope.netFare = netFare; //TBD
        $scope.originalFare = originalFare; //TBD
        $scope.expired = false;
        $scope.expiryDate = new Date(expirytime);
        $scope.hotelDetail = hotelDetail; //tbd
        $scope.totalRoom = totalRoom; //TBD
        $scope.nights = nights; //TBD
        $scope.country = country; //TBD
        ...
        $scope.roomType = $scope.hotelDetail.room[0].roomName; //tbd
        $scope.roomService = 0; //tbd
        $scope.currentPage = 1;
        $scope.stepClass = '';
