...
app.controller('paymentController', [
    '$http', '$scope', '$location', '$log', function ($http, $scope, $location, $log) {
        ...
        angular.element(document).ready(function () {
            $scope.UniqueCodePaymentConfig.GetUniqueCode($scope.trxId);
        });
        ...
        $scope.creditsAvailable = creditsAvailable;
        $scope.creditsVoucherCode = creditsVoucherCode;
        $scope.buyerInfo = {};
        var date = new Date();
        ...
        //                        $scope.UniqueCodePaymentConfig.GetUniqueCode($scope.trxId, '', $scope.CreditCard.Number.substring(0, 6));
        //                    } else {
        ...
        $scope.UniqueCodePaymentConfig = {
            UniqueCode: 0,
            Token: '',
            GetUniqueCode: function (trxId, voucherCode, bin) {
                ...
                if (authAccess == 1 || authAccess == 2) {
                    $http({
                        ...
                        if (refreshAuthAccess() && $scope.trial < 4) //refresh cookie
                        {
                            $scope.UniqueCodePaymentConfig.GetUniqueCode($scope.trxId, voucherCode);
                        }
                        else {
        ...
        $scope.useCreditsFunction = function () {
            var currentState = $("input.useCredits").prop("checked");
            $("input.useCredits").prop("checked", !currentState);
            $("input.useVoucher").prop("checked", false);
            $scope.voucher.code = '';
            $("form[name='voucherForm']").hide(200);
            if (currentState) {
                $scope.voucher.confirmedCode = '';
            } else {
                $scope.voucher.confirmedCode = $scope.creditsVoucherCode;
                $scope.voucher.displayName = 'Credits';
                $scope.voucher.amount = $scope.creditsAvailable;
            }
            console.log($scope.voucher);
        }

        $scope.useVoucherFunction = function () {
            var currentState = $("input.useVoucher").prop("checked");
            $("input.useVoucher").prop("checked", !currentState);
            $("input.useCredits").prop("checked", false);
            $scope.voucher.code = '';
            $scope.voucher.confirmedCode = '';
            ...
            if (currentState) {
                ...
                $("form[name='voucherForm']").show(200);
            }
            console.log($scope.voucher);
        }

        //$("input.useCredits").click(function (e) {
        //    useCreditsFunction();
        //});

        //$("input.useVoucher").click(function (e) {
        //    useVoucherFunction();
        //});

        //$("label.useCredits").click(function () {
        //    useCreditsFunction();
        //});

        //$("label.useVoucher").click(function () {
        //    useVoucherFunction();
        //});
                ...
                if (authAccess == 1 || authAccess == 2) {
                    $http({
                        ...
                        if (returnData.data.discount > 0) {
                            ...
                            $scope.binDiscount.text = '';
                            $scope.methodDiscount.text = '';
                            $scope.UniqueCodePaymentConfig.GetUniqueCode($scope.trxId, $scope.voucher.code);
        ...
        $scope.pay = {
            url: $scope.isRsv ? FlightPayConfig.Url : CartCheckoutConfig.Url,
            ...
            send: function () {
                ...
                switch ($scope.paymentMethod) {
                    ...
                    case "MandiriClickPay":
                        ...
                        var trxIdLast5 = $scope.trxId + "";
                        trxIdLast5 = trxIdLast5.substr(trxIdLast5.length - 5);
                        ...
                        if (returnData.data.status == '200') {
                            ...
                            switch (returnData.data.error) {
                                ...
                                case 'ERPPAY03':
                                    ...
                                    $('form#rsvno input#rsvno-input').val($scope.trxId);
