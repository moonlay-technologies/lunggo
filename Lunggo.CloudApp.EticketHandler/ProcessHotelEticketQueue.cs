...
{
    public class ProcessHotelEticketQueue
    {

        public static async Task ProcessQueue()
        {
            var queue = QueueService.GetInstance().GetQueueByReference("HotelEticket");
            ...
            sw.Start();
            var eticketTemplate = templateService.GenerateTemplate(reservation, "HotelEticket");
            sw.Stop();
            ...
            var emailQueue = queueService.GetQueueByReference("HotelEticketEmail");
            emailQueue.AddMessage(new CloudQueueMessage(rsvNo));

            Trace.WriteLine("Pushing Hotel Eticket Notification for RsvNo " + rsvNo + "...");
            //HotelService.PushEticketIssuedNotif(rsvNo);
