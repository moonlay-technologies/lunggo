...
namespace Lunggo.WebJob.EmailQueueHandler.Function
{
    ...
    {
        public static void HotelEticketEmail([QueueTrigger("Hoteleticketemail")] string rsvNo)
        {
            var env = ConfigManager.GetInstance().GetConfigValue("general", "environment");
            var envPrefix = env != "production" ? "[" + env.ToUpper() + "] " : "";

            var sw = new Stopwatch();
            Console.WriteLine("Processing Hotel Eticket Email for RsvNo " + rsvNo + "...");
            ...
            Console.WriteLine("Sending Hotel Eticket Email...");
            mailService.SendEmail(summary, mailModel, "HotelEticketEmail");
            //HotelService.GetInstance().UpdateIssueProgress(rsvNo, "Eticket Email Sent. Ticket Issuance Complete.");
            
            Console.WriteLine("Done Processing Hotel Eticket Email for RsvNo " + rsvNo);
        }
    }
}
