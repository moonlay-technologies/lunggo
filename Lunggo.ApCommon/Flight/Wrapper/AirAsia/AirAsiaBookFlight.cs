            ...
            internal BookFlightResult BookFlight(FlightBookingInfo bookInfo)
            {
                ...
                catch
                {
                    return new BookFlightResult
                    {
                        ...
                        ErrorMessages = new List<string> { "[AirAsia] Error while splitting the fareid" }
                    };
                }
                     ...
                     searchResponse.StatusCode != HttpStatusCode.Redirect))
                {
                    ...
                    return new BookFlightResult
                    {
                        ...
                        Errors = new List<FlightError> { FlightError.TechnicalError },
                        ErrorMessages = new List<string> { "[AirAsia] Error in requesting at Search.aspx.Unexpected absolute path response or status code || " + searchResponse.StatusCode + " " + searchResponse.ResponseUri + " || " + searchResponse.Content }
                    };
                }
                     ...
                     selectResponse.StatusCode != HttpStatusCode.Redirect))
                {
                    return new BookFlightResult
                    {
                        ...
                        Errors = new List<FlightError> { FlightError.FareIdNoLongerValid },
                        ErrorMessages = new List<string> { "[AirAsia] Error in requesting at Select.aspx. Unexpected absolute path response or status code || " + selectResponse.StatusCode + " " + selectResponse.ResponseUri + " || " + selectResponse.Content }
                ...
                foreach (var passenger in bookInfo.Passengers.Where(p => p.Type == PaxType.Adult))
                {
                    ...
                    postData +=
                        ...
                        @"&CONTROLGROUP_OUTERTRAVELER%24CONTROLGROUPTRAVELER%24PassengerInputTravelerView%24DropDownListNationality_" + i + "=" + passenger.Nationality +
                ...
                foreach (var passenger in bookInfo.Passengers.Where(p => p.Type == PaxType.Infant))
                {
                    postData +=
                        ...
                        @"&CONTROLGROUP_OUTERTRAVELER%24CONTROLGROUPTRAVELER%24PassengerInputTravelerView%24DropDownListNationality_" + i + "_" + i + "=" + passenger.Nationality +
                     ...
                     travelerResponse.StatusCode != HttpStatusCode.Redirect))
                {
                    return new BookFlightResult
                    {
                        ...
                        {
                            "[AirAsia] Error while requesting at Traveler.aspx. Unexpected absolute path response or status code || " + travelerResponse.StatusCode + " " + travelerResponse.ResponseUri + " || " + ((CQ) travelerResponse.Content)["#errorSectionContent"]
                        }
                    };
                }
                     ...
                     unitMapResponse.StatusCode != HttpStatusCode.Redirect))
                {
                    return new BookFlightResult
                    {
                        ...
                        ErrorMessages = new List<string> { "[AirAsia] Error while requesting at UnitMap.aspx. Unexpected absolute path response or status code || " + unitMapResponse.StatusCode + " " + unitMapResponse.ResponseUri + " || " + unitMapResponse.Content }
                    };
                }
                ...
                catch
                {
                    return new BookFlightResult
                    {
                        ...
                        Errors = new List<FlightError> { FlightError.TechnicalError },
                        ErrorMessages = new List<string> { "[AirAsia] Web Layout Changed!, Error while revalidate in last step" }
                    };
                }
                ...
                while (paymentResponse.StatusCode == HttpStatusCode.Forbidden && whileCounter < 10)
                {
                    ...
                    //    },
                    //    Errors = new List<FlightError> { FlightError.TechnicalError },
                    //    ErrorMessages = new List<string>{"[AirAsia] Forbidden!"}
                    //};
                }
                     ...
                     paymentResponse.StatusCode != HttpStatusCode.Redirect))
                {
                    return new BookFlightResult
                    {
                        ...
                        Errors = new List<FlightError> { FlightError.FailedOnSupplier },
                        ErrorMessages = new List<string> { "[AirAsia] Error while select hold payment. Unexpected absolute path response or status code " + paymentResponse.StatusCode + " " + paymentResponse.ResponseUri + " || " + paymentResponse.Content }
                    };
                }
                ...
                if (waitResponse.ResponseUri.AbsolutePath != "/Itinerary.aspx" || (waitResponse.StatusCode != HttpStatusCode.OK && waitResponse.StatusCode != HttpStatusCode.Redirect))
                {
                    ...
                    if (bookingId == "" || bookingId == null)
                    {
                        return new BookFlightResult
                        {
                            ...
                            Errors = new List<FlightError> { FlightError.FailedOnSupplier },
                            ErrorMessages = new List<string> { "[AirAsia] Failed to get booking id || " + waitResponse.StatusCode + " " + waitResponse.ResponseUri + " || " + waitResponse.Content }
                        };
                    }
                    else
                    {
                ...
                catch
                {
                    return new BookFlightResult
                    {
                        ...
                        Errors = new List<FlightError> { FlightError.TechnicalError },
                        ErrorMessages = new List<string> { "[AirAsia] Web Layout Changed! Error while getting bookingid and timelimit " }
                    };
                }
            }
        }
