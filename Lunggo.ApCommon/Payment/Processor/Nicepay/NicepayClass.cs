    ...
    internal partial class PaymentProcessorService
    {
        internal class NicepayClass : System.Web.UI.Page
        {
            internal NicepayResponse CreateVA(NicepayModel Nicepay)
            {
                ...
                Nicepay.iMid = NicepayWrapper.MerchantId;
                ...
                string Data = Post_Http(SingleString, API_Url);
            ...
            internal NicepayResponse ChargeCard(NicepayModel Nicepay)
            {
                ...
                string ResultString = Post_Http(SingleString, API_Url);
            ...
            static string merchantToken(NicepayModel Nicepay, string RequestType)
            {
                ...
                {
                    MerchantStr = Encoding.ASCII.GetBytes(NicepayWrapper.MerchantId + Nicepay.tXid + Nicepay.amt +
                                                          NicepayWrapper.MerchantKey);
                }
                else if (RequestType == "registerCustomer" || RequestType == "retrieveVaInfo")
                {
                    MerchantStr =
                        Encoding.ASCII.GetBytes(NicepayWrapper.MerchantId + Nicepay.customerId +
                                                NicepayWrapper.MerchantKey);
                }
                else if (RequestType == "listDepositCustomerId")
                {
                    MerchantStr = Encoding.ASCII.GetBytes(NicepayWrapper.MerchantId + Nicepay.customerId +
                                                          Nicepay.startDt + NicepayWrapper.MerchantKey);
                }
                else if (RequestType == "listDepositVa")
                {
                    MerchantStr = Encoding.ASCII.GetBytes(NicepayWrapper.MerchantId + Nicepay.vacctNo +
                                                          Nicepay.startDt + NicepayWrapper.MerchantKey);
                }
                else
                {
                    MerchantStr = Encoding.ASCII.GetBytes(NicepayWrapper.MerchantId + Nicepay.referenceNo +
                                                          Nicepay.amt + NicepayWrapper.MerchantKey);
                }
            ...
            {
                string API_req = NicepayWrapper.Endpoint;
            ...
            internal string PrintError(string ErrorNo)
            {
                ...
                switch (ErrorNo)
                {
                    ...
                    case "08":
                        ...
                        break;
                    case "09":
                        getError = new string[1, 2]
                            {{"10009", "(billingPhone) is not set. Please set (billingPhone)."}};
                        break;
                    case "10":
                        getError = new string[1, 2]
                            {{"10010", "(billingEmail) is not set. Please set (billingEmail)."}};
                        break;
                    case "11":
                    ...
                    case "13":
                        getError = new string[1, 2]
                            {{"10013", "(billingState) is not set. Please set (billingState)."}};
                        break;
                    case "14":
                        getError = new string[1, 2]
                            {{"10014", "(billingCountry) is not set. Please set (billingCountry)."}};
                        break;
                    case "15":
                    ...
                    case "16":
                        getError = new string[1, 2]
                            {{"10016", "(deliveryPhone) is not set. Please set (deliveryPhone)."}};
                        break;
                    case "17":
                        getError = new string[1, 2]
                            {{"10017", "(deliveryAddr) is not set. Please set (deliveryAddr)."}};
                        break;
                    case "18":
                        getError = new string[1, 2]
                            {{"10018", "(deliveryCity) is not set. Please set (deliveryCity)."}};
                        break;
                    case "19":
                        getError = new string[1, 2]
                            {{"10019", "(deliveryState) is not set. Please set (deliveryState)."}};
                        break;
                    case "20":
                        getError = new string[1, 2]
                            {{"10020", "(deliveryPostCd) is not set. Please set (deliveryPostCd)."}};
                        break;
                    case "21":
                        getError = new string[1, 2]
                            {{"10021", "(deliveryCountry) is not set. Please set (deliveryCountry)."}};
                        break;
                    case "22":
                    ...
                    case "23":
                        getError = new string[1, 2]
                            {{"10023", "(dbProcessUrl) is not set. Please set (dbProcessUrl)."}};
                        break;
                    case "24":
                    ...
                    case "28":
                        getError = new string[1, 2]
                            {{"10028", "(merchantToken) is not set. Please set (merchantToken)."}};
                        break;
                    case "29":
